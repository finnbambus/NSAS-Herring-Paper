// ─── RHO TYPST TEMPLATE – PUBLIC API ──────────────────────────────────────────
// Import everything through this single file:
//   #import "lib/rho.typ": *

#import "theme.typ":        accent, accent-bg, font-sans, font-serif, font-mono, text-muted, text-faint
#import "cover.typ":        show-cover
#import "article-header.typ": show-article-header, abstract-box, info-box, keywords-line
#import "layout.typ":       show-layout
#import "envs.typ":         rho-box, note-block
