...
// show-article-header() renders the first-page block above the body.
// Every field is optional; pass none to hide it.
...
#let info-box(
  ...
  if email       != none { text(fill: accent, style: "italic", "E-mail: ");     email;       linebreak() }
...
#let show-article-header(
  title:        "",
  authors:      (),   // array of (name, affiliation) pairs or plain strings
  date:         none,
  abstract:     none,
  keywords:     none,
  show-info:    true,
  // info-box fields (forwarded):
  supervisors: none, email: none, submitted: none, defended: none,
  accepted: none, published: none, doi: none, license: none,
) = {
  // Title
  set par(justify: false, leading: 0.4em)
  text(fill: accent, weight: "bold", size: 20pt, font: font-sans, title)
  v(-5pt)
  ...
  if authors.len() > 0 {
    ...
    authors.map(a => {
      if type(a) == str { a }
      else { a.name + super(a.at("affil", default: "")) }
    }).join(", ")
    v(3pt)
  }
  ...
  if show-info {
    info-box(
      supervisors: supervisors, email: email,
